...
wordCount s =
  "Number of lines: " ++ (show . length . lines $ s)
  ++ "\nNumber of empty lines: " ++ (show $ foldl' (\count l -> if length (words l) == 0 then 1 + count::Integer else count) 0 (lines s))
  ++ "\nNumber of words: " ++ (show . length . words $ s)
  ++ "\nNumber of unique words: " ++ (show . length . uniqueWords . words $ s)
  ++ "\nNumber of words followed by themselves: " ++ (show . sum . map (\pair -> if fst pair == snd pair then 1 else 0) . adjacents . words $ s)
  ++ "\nLength of the longest line: " ++ (show . length . longestText . lines $ s)
  ++ "\n"
  where
    uniqueWords xs = go [] xs
      where
        go ys [] = ys
        go ys (x:xs)
          | x `elem` ys = go ys xs
