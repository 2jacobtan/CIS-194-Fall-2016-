...
labelTree t = runSupply nats $ go0 t
  where
    go0 :: Num s => Tree a -> Supply s (Tree s)
    go0 t = pureSupply $ go t
    go (Leaf a) = Leaf 0
    go (Node t1 t2) = go t1 `seq` Node (go t1) $ go t2
