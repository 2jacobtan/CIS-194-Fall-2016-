...
handleEvent (KeyPress key) (State from dir boxList) -- FIXME! -- doing
...
validatedMove (State from dir boxList)
  | maze to == Ground || maze to == Storage = State to dir boxList
  | maze to == Box && (maze to2 == Ground || maze to2 == Storage) = boxMove
  | otherwise = State from dir boxList
  where
    to = adjacentCoord dir from
    to2 = adjacentCoord dir to
    boxMove :: State
    boxMove = (State to dir (mapList (moveFromTo to to2) boxList))
